...
final float Dens=0.015;    //0.5 or so also posible

final boolean withMe=true; //self awareness - what about own state of cell in new state calculation?

int[] WorldOld=new int[WorldSize]; //We need two arrays/lists for the old  
...
void draw() // modifies extern t,WorldOld,WorldNew
{
  if(t>994) return;
  visualisation();
