...
void draw()
{
  ...
  if(StepCounter % STEPSperVIS == 0 ) //But when model is running, visualisation shoud be done from time to time
...
void statusLine()
{
  ...
  fill(0);
  textAlign(LEFT, TOP); //text(meanStress+"  "+liveCount,0,side*cwidth);
  textAlign(LEFT, BOTTOM); text(StepCounter+")  Fps:"+ frameRate,0,side*cwidth+STATUSHEIGH-2);
}
