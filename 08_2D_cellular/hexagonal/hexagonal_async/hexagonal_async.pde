...
int   WorldSize=50; //How many elements do we want in a line?
float IDens=0.00;   //Initial density in the array (0 means initialization in the middle)
int   Div=5;

int[][] World=new int[WorldSize][WorldSize]; 

float CellSize=10; //We will use the ability to specify screen coordinates as `float`
...
void visualise_connections() ///< Connection visualization
{
  for(int i=1;i<World.length-1;i++)
    ...
    {    
      ...
      int add   = (j%2==0 ?  right :  left  ); // even vs. odd
      stroke(255,0,255);
