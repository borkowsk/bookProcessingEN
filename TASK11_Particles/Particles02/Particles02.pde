...
{
  float xitmp=xspeed[i];
  float yitmp=yspeed[i];
  
  if(collision[i]==false && collision[j]==false){
     xspeed[i] = xspeed[j];
     yspeed[i] = yspeed[j];
     xspeed[j] = xitmp;
     yspeed[j] = yitmp;
  ...
  /*
  if(xspeed[i]==0)xspeed[i] = random(maxspeed*0.2,maxspeed)*(random(1.0)<0.5?-1:1);
  if(yspeed[i]==0)yspeed[i] = random(maxspeed*0.2,maxspeed)*(random(1.0)<0.5?-1:1);
  if(xspeed[j]==0)xspeed[j] = random(maxspeed*0.2,maxspeed)*(random(1.0)<0.5?-1:1);
  if(yspeed[j]==0)yspeed[j] = random(maxspeed*0.2,maxspeed)*(random(1.0)<0.5?-1:1);
  */
}
