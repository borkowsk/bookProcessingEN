...
boolean isPositionFree(int i)
{
  for (int j=0; j<circles; j++)
    if ( j!=i && dist(xpos[i], ypos[i], xpos[j], ypos[j])<=diameter )
      return false;
  return true;
}


void setup() // uses global xpos,ypos,xspeed,yspeed,collision,circolor
{
  size(900, 900);
  smooth();
  background(0);
...
void movement() // uses global xpos,ypos,xspeed,yspeed,collision,circolor
{
  for (int i=0; i<circles; i++) {
    ypos[i]+=yspeed[i];
    xpos[i]+=xspeed[i];
...
void bordersReaction() // uses global xpos,ypos,xspeed,yspeed,collision,circolor
{
  for (int i=0; i<circles; i++)
    if (torus)
    {
...
void findcollisions() //< uses global xpos,ypos,xspeed,yspeed,collision,circolor
{
  ...
  for (int i=0; i<circles; i++) 
    collision[i]=false; // Clear collisions
}

/// SIMPLIFIED elastic rebound, i.e. the physical reaction to collisions
void physicalCollision(int i, int j) // uses global xspeed,yspeed,collision
{
...
void reaction(int i, int j) // uses global xpos,ypos,xspeed,yspeed,collision,circolor
{
  if (collision[i]==false) {
    //... YOUR CODE
    //eg.:
